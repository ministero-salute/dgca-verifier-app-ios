...
class TestValidityCheckTests: XCTestCase {
    ...
    func testValidNegativeTest() {
        ...
        guard let testDate = hcert.testDate?.toVaccineDate else { return }
        let passedHours = Calendar.current.dateComponents([.hour], from: Date(), to: testDate)
        guard var passedHoursInt = passedHours.day else { return }

        if passedHoursInt < 0{
            passedHoursInt = passedHoursInt * -1 + 48
        }
        else {
            passedHoursInt = passedHoursInt + 48
        }
        
        let passedHoursString = passedHoursInt.stringValue
        
        let testSettingStartDay = Setting(name: Constants.rapidStartHoursKey, type: "GENERIC", value: "0")
        let testSettingEndDay = Setting(name: Constants.rapidEndHoursKey, type: "GENERIC", value: passedHoursString)
    ...
    func testValidRapidTestDate() {
        ...
        if passedHoursInt < 0{
            passedHoursInt = passedHoursInt * -1 + 1
        }
        else {
            passedHoursInt = passedHoursInt + 1
        }
